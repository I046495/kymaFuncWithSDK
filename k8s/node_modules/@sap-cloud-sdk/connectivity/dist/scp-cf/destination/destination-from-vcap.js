...
async function getDestinationFromServiceBinding(options) {
    ...
    const destination = (await retrieveDestinationFromCache(retrievalOptions)) ||
...
async function retrieveDestinationFromCache(options) {
    if (options.useCache) {
        return destination_cache_1.destinationCache.retrieveDestinationFromCache((0, jwt_1.decodeOrMakeJwt)(options.jwt), options.destinationName, 'tenant');
    }
}
