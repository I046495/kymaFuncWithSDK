"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.entityDeserializer = void 0;
const odata_common_1 = require("@sap-cloud-sdk/odata-common");
const response_data_accessor_1 = require("../request-builder/response-data-accessor");
const extract_odata_etag_1 = require("../extract-odata-etag");
/**
 * Entity deserializer instance for v2 entities.
 * See {@link @sap-cloud-sdk/odata-common!EntityDeserializer} for the provided methods.
 * @param deSerializers - DeSerializer.
 * @returns EntityDeserializer.
 */
function entityDeserializer(deSerializers) {
    return (0, odata_common_1.entityDeserializer)(deSerializers, extract_odata_etag_1.extractODataEtag, response_data_accessor_1.getLinkedCollectionResult);
}
exports.entityDeserializer = entityDeserializer;
//# sourceMappingURL=entity-deserializer.js.map