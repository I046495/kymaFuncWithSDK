...
exports.DestinationFromServiceRetriever = exports.getDestinationFromDestinationService = void 0;
...
class DestinationFromServiceRetriever {
    ...
    async getInstanceAndSubaccountDestinations(accessToken) {
        const [instance, subaccount] = await Promise.all([
            (0, destination_service_1.fetchInstanceDestinations)((0, environment_accessor_1.getDestinationServiceCredentials)().uri, accessToken, this.options),
            (0, destination_service_1.fetchSubaccountDestinations)((0, environment_accessor_1.getDestinationServiceCredentials)().uri, accessToken, this.options)
        ]);
        return {
            instance,
            subaccount
    ...
    async fetchDestinationByToken(jwt) {
        return (0, destination_service_1.fetchDestination)((0, environment_accessor_1.getDestinationServiceCredentials)().uri, jwt, this.options);
    }
    getExchangeTenant(destination) {
        ...
        const subdomainSubscriber = (0, subdomain_replacer_1.getIssuerSubdomain)(this.subscriberToken?.userJwt?.decoded);
        const subdomainProvider = (0, subdomain_replacer_1.getIssuerSubdomain)(this.providerServiceToken?.decoded);
...
Possible alternatives for such technical user authentication are BasicAuthentication, OAuth2ClientCredentials, or ClientCertificateAuthentication`);
    ...
    isSubscriberNeeded() {
        ...
        if (this.options.selectionStrategy.toString() === destination_selection_strategies_1.alwaysProvider.toString()) {
            return false;
        }
        return true;
    }
